...
class UpdateOrderRequest extends FormRequest
{
    ...
    public function messages(): array
    {
        ...
        // ➕ Add dynamic "required" messages for material fields
        foreach ($this->input('order_items', []) as $index => $item) {
            if (isset($item['work_type']) && $item['work_type'] === 'New from Material') {
                $messages["order_items.$index.material_type.required"] = "Material type is required for item #$index because work type is 'New from Material'.";
                $messages["order_items.$index.material_code.required"] = "Material code is required for item #$index because work type is 'New from Material'.";
            }
        }

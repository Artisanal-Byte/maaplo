...
let featuresArray = [];
try {
    featuresArray = Object.values(JSON.parse(props.subscriptionPlan.features || '{}'));
} catch (e) {
    featuresArray = [];
}

const form = useForm({
    plan_title: props.subscriptionPlan.plan_title,
    plan_description: props.subscriptionPlan.plan_description || '',
    plan_price: props.subscriptionPlan.plan_price,
    plan_currency: props.subscriptionPlan.plan_currency,
    features: featuresArray.length > 0 ? featuresArray : [''],
    visibility: Boolean(props.subscriptionPlan.visibility),
    user_limit: props.subscriptionPlan.user_limit,
});
...
const submitForm = () => {
    ...
    form.put(route('subscription-plans.update', props.subscriptionPlan.id), {
        onSuccess: () => {
                        ...
                        <div class="flex gap-4">
                            <label>
                                ...
                                <div :class="[
                                    ...
                                    form.visibility === true ? 'bg-primary text-white' : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-100',
                                ]">
                                    Visible
                                </div>
                            </label>
                            ...
                            <label>
                                ...
                                <div :class="[
                                    'px-4 py-1 rounded border text-sm cursor-pointer',
                                    form.visibility === false ? 'bg-primary text-white' : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-100',
                                ]">
                                    Hidden
                                </div>
                            </label>

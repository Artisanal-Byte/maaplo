<script setup>
const model = defineModel('model')  // 'model' matches v-model:model
</script>

<template>
  <label class="relative inline-flex items-center cursor-pointer">
    <input type="checkbox" v-model="model" class="sr-only peer">
    <div class="w-11 h-6 bg-gray-300 rounded-full peer dark:bg-gray-700 peer-checked:bg-primary transition duration-300"></div>

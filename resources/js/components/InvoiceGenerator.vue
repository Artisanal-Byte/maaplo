    ...
    async function downloadPDF() {
        ...
        doc.text("TAX INVOICE", rightX, contentY - 20, { align: "left" });
        ...
        const orgState = props.order?.organization?.state?.trim()?.toLowerCase();
        const customerState = props.order?.customer?.state?.trim()?.toLowerCase();
        const orgGstNumber = props.order?.organization?.gst_number?.trim();

        let cgst = 0, sgst = 0, igst = 0;
        let tax = 0;
        let payable = total;

        if (orgGstNumber && orgGstNumber !== "") {
            if (orgState && customerState && orgState === customerState) {
                // Intra-state: CGST + SGST
                cgst = total * (gstRate / 2);
                sgst = total * (gstRate / 2);
            } else {
                // Inter-state (or state missing): IGST
                igst = total * gstRate;
            }
            tax = cgst + sgst + igst;
            payable = total + tax;
        ...
        tableData.push(
            [
                { content: "Total", styles: { lineWidth: 0, fontStyle: "bold", halign: "left" } },
                { content: formatCurrency(total), styles: { lineWidth: 0, halign: "left" } }
            ]
        );

        // Add GST rows only if org has GST
        if (orgGstNumber && orgGstNumber !== "") {
            if (cgst || sgst) {
                tableData.push(
        ...
        tableData.push(
            [
                { content: "Total Payable Amount", styles: { fontStyle: "bold", halign: "left" } },
                { content: formatCurrency(payable), styles: { fontStyle: "bold", halign: "left" } }
            ]
        );

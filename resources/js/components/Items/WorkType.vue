...
const currentItem = computed(() => {
    return isEditing.value
        ? props.order.order_items[props.currentEditIndex]
...
watch(
    ...
    ([workType, materialCost, stitchingCost, alteringCost]) => {
        let total = 0;
        ...
        if (workType === 'New from Material') {
            total = m + s;
            currentItem.value.material_cost = m;
            currentItem.value.stiching_cost = s;
        }
        ...
        if (workType === 'Only Altering') {
            ...
            currentItem.value.altering_cost = a;
        }

...
const measurements = ref<Record<string, string>>({ unit: 'in' });

onMounted(() => {
    if (props.measurements && Object.keys(props.measurements).length > 0) {
        measurements.value = {
            ...props.measurements,
...
watch(measurements, (val) => {
    if (JSON.stringify(val) !== JSON.stringify(props.measurements)) {
                ...
                <li>
                    <div class="grid grid-col-1 lg:grid-cols-2 gap-x-4 gap-y-4 ml-2 mt-4 text-black font-lato text-sm">
                        ...
                        <div class="flex items-center gap-4 col-span-2 sm:col-span-1"
                            v-for="(value, key) in JSON.parse(toAsk)" :key="key">
